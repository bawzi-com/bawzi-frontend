...
// Se já tiver uma interface definida, pode manter a sua. 
// Esta é apenas para garantir que o TypeScript não reclama.
...
export default function UserProfileCard({ user }: UserProfileCardProps) {
  // Cores dinâmicas para o Badge do Nível (Tier)
  const getTierStyle = (tier: number = 1) => {
    switch (tier) {
      case 4: return 'bg-slate-900 text-yellow-400 border-slate-800 shadow-[0_0_10px_rgba(250,204,21,0.2)]'; // Elite
  ...
  // Nomes dos Tiers
  const getTierName = (tier: number = 1) => {
    switch (tier) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${getTierStyle(user.tier)}`}>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-slate-700">
            {vagasUsadas} <span className="text-slate-400 font-medium">de {vagasTotais}</span>
          </span>
        </div>
        ...
        {percentagemUso >= 100 && (
          <p className="text-[10px] text-amber-600 font-bold mt-0.5 flex items-center gap-1">
            <span>⚠️</span> Limite de vagas atingido
          </p>
        )}
      </div>
      ...
      <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 flex items-start gap-3 transition-colors hover:bg-slate-100/50">
        ...
        <div className="flex flex-col min-w-0 justify-center min-h-[32px]">
          ...
          <span className="text-sm font-bold text-slate-700 truncate">
            {user.company?.fantasy_name || user.company?.name || 'Sem empresa vinculada'}
          </span>
        </div>
      </div>
