...
/** Uma evidência de preço homologado usada na comparação POR ITEM (23/09/2026). */
export interface ItemComparadoEvidencia {
  valor: number;
  descricao?: string;
  fornecedor?: string;
  orgao?: string;
 ...
 *  tem código, por descrição quando não tem. Mediana, coerência e selo são
 *  calculados no backend só sobre as evidências deste item. */
export interface ItemComparado {
  numero_item?: number | string | null;
  descricao: string;
  quantidade?: number;
  unidade?: string;
  ...
  selo?: { nivel: 'forte' | 'bom' | 'parcial' | 'fraco'; texto: string; detalhe?: string };
  evidencias?: ItemComparadoEvidencia[];
...
export interface PricingIntelligenceData {
  ...
  /** Preço de referência do próprio edital — NÃO é evidência de mercado. */
  precoReferenciaEdital?: { mediana: number; minimo: number; maximo: number; itens: number; descricao: string };
  ...
  /** Itens do edital (PNCP) — nº, descrição, quantidade, estimativa do órgão, CATMAT. */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  itensEdital?: Array<Record<string, any>>;
  /** Comparação por item (23/09/2026) — ver ItemComparado. */
  itensComparados?: ItemComparado[];
  /** Itens do edital que ficaram fora da comparação por causa do teto por análise. */
  itensNaoComparados?: number;
  ...
  fonteConfiabilidadeDetalhe?: {
    ...
    fonte_mais_forte_presente: string;
  };
  /** True quando o deságio não pôde ser apurado (campos vêm null). */
...
/** Por que um lote do radar veio vazio. Vem do backend junto com a lista.
...
/** Nome de empresa reduzido ao que identifica: sem acento, sem sufixo societário
 *  (LTDA, ME, EPP, S/A...), sem pontuação — para cruzar o FORNECEDOR de uma
 *  evidência de preço com os concorrentes já mapeados no radar. */
const normalizarNomeEmpresa = (s: unknown): string =>
  String(s || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .toUpperCase()
    .replace(/\b(LTDA|LIMITADA|ME|EPP|EIRELI|S\/A|SA|SS|CIA|COMERCIO|COMERCIAL|E|DE|DO|DA|DOS|DAS)\b/g, ' ')
    .replace(/[^A-Z0-9 ]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
export default function CompetitorWarRoom({
  ...
  // Itens do edital × mercado — quais itens estão com a lista de comparáveis aberta.
  ...
  const itensLotes = useMemo(() => {
    ...
    if (dadosMinerados.valorGlobal > 0) {
      // ANTES: `|| 19.7` — um deságio inventado no FRONTEND quando o backend
      // não apurava nenhum. Ficava indistinguível de um número calculado.
      return [{ numero: "Lote", produto: nomeObjetoReal, valor_estimado_raw: dadosMinerados.valorGlobal, quantidade: dadosMinerados.quantidade, desagioPreditivoOrgao: pricing?.desagioPreditivoOrgao ?? 0 }];
    }
  ...
  // Resultado da reapuração sobrescreve as props. Enquanto for null, valem os
  ...
  // Teto próprio para esta chamada. O `apiFetch` do projeto usa
  // DEFAULT_TIMEOUT_MS = 20_000, que é curto demais aqui: só o radar tem 45s de
  // orçamento no backend, mais os enriquecimentos de CNPJ. Com 20s, esta rota
  // era abortada pelo navegador antes de o servidor terminar — sempre, no
  // caminho lento, que é exatamente o caminho em que alguém clica no botão.
  //
  // Passar um `signal` próprio desarma o teto padrão sem tocar no apiClient:
  // `fetchWithTimeout` faz `signal: options.signal ?? controller.signal`, então
  // o timer interno passa a abortar um controller que não está preso a nada.
  const TETO_REAPURACAO_MS = 90_000;
  ...
  const reapurarConcorrentes = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        let msg = `O servidor recusou a atualização (HTTP ${resp.status}).`;
        try {
          const corpo = await resp.json();
          msg = corpo?.detail?.mensagem || corpo?.detail || msg;
        } catch { /* corpo não-JSON: fica a mensagem com o status */ }
      ...
      // ⚠️ Aqui havia uma frase única — "Falha de conexão ao consultar o PNCP" —
      // para qualquer exceção. Ela mentia duas vezes no caso mais comum: o que
      // acontecia era um AbortError do próprio navegador por tempo esgotado, sem
      // relação com conexão e sem relação com o PNCP. Converter causas distintas
      // numa mensagem só é a mesma doença que passamos a sessão corrigindo no
      // backend: some com a informação de que se precisa para agir.
      const abortou = err instanceof DOMException && err.name === 'AbortError';
      if (abortou) {
        setErroReapuracao(
          `A consulta passou de ${Math.round(TETO_REAPURACAO_MS / 1000)}s e foi interrompida. ` +
          'O PNCP oscila ao longo do dia — tente de novo em alguns minutos.'
        );
      } else {
        const detalhe = err instanceof Error && err.message ? ` (${err.message})` : '';
        setErroReapuracao(`Não foi possível falar com o servidor${detalhe}.`);
      }
    } finally {
      clearTimeout(timer);
      setReapurando(false);
  ...
  const vereditoCompetitivo = useMemo(() => {
    ...
    // O backend agora manda o motivo exato quando não consegue apurar o
    // deságio, em vez de devolver um número padrão disfarçado de cálculo.
    const motivoIndisponivel = String(pricing?.desagioIndisponivelMotivo || '');
    const detalheFonte = pricing?.fonteConfiabilidadeDetalhe;
    const dadoFragil =
      motivoIndisponivel
        ? motivoIndisponivel
        : pricing?.usandoDefaultFallback
          ? 'Deságio não apurado para este objeto — não há base histórica suficiente.'
          : amostra === 0
            ? 'Sem amostra de preço unitário confiável para este objeto.'
            : detalheFonte && detalheFonte.participacao_pct < 60
              ? `Amostra de ${detalheFonte.amostra} preço(s), com origem mista (${detalheFonte.participacao_pct}% de ${detalheFonte.dominante}). Use como orientação.`
              : fonte !== 'PNCP_HOMOLOGADO'
                ? `A fonte predominante do preço (${fonte || 'não identificada'}) não é homologada no PNCP; use como orientação, não como certeza.`
                : `Mediana de ${amostra} preço(s) homologado(s) no PNCP, sujeita à similaridade do objeto.`;
      ...
      {view === 'radar' && (
        <div ref={operationRef} className="flex flex-col gap-6 animate-in fade-in duration-300">
          ...
          {/* ━━━ SEM BASE PARA INTELIGÊNCIA DE PREÇO ━━━
              ...
              Em vez de sumir com a seção sem explicação, dizemos por quê. */}
          {pricing && !pricing.desagioPreditivoOrgao && !pricing.nivelAmeaca && !pricing.perfilVencedor && (
            <div className="border border-slate-200 bg-slate-50 rounded-[2rem] p-6 md:p-8">
              <div className="flex items-center gap-2 mb-2">
                <Lightbulb size={18} className="text-slate-400" />
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest">Inteligência de Preço do Órgão</h3>
              </div>
              <p className="text-sm font-semibold text-slate-600 leading-relaxed">
                {pricing.desagioIndisponivelMotivo
                    ...
                    {pricing.fonteConfiabilidadeDetalhe &&
                     Object.keys(pricing.fonteConfiabilidadeDetalhe.distribuicao || {}).length > 1 && (
                      <p className="text-[9px] text-slate-500 font-bold mb-1.5 leading-relaxed">
                        Composição:{' '}
                        {Object.entries(pricing.fonteConfiabilidadeDetalhe.distribuicao)
                          .map(([fonte, n]) => `${n}× ${fonte.replace(/_/g, ' ').toLowerCase()}`)
                          .join(' · ')}
                      </p>
                    )}
                    {/* Deságio não apurado — antes aqui aparecia um número padrão */}
                    {pricing.usandoDefaultFallback && (
                      <p className="text-[9px] text-amber-600 font-bold mb-1 flex items-center gap-1">
                        <span>⚠</span> Deságio não apurado — sem base histórica
                      </p>
                    )}
                    <div>
          ...
          {(() => {
            ...
            const seloCls: Record<string, string> = {
              ...
              bom: 'bg-teal-100 text-teal-700',
              parcial: 'bg-amber-100 text-amber-700',
              fraco: 'bg-rose-100 text-rose-700',
            };
            const nomesConcorrentes = new Set(
              [...listaNacional, ...listaRegional]
                .map(c => normalizarNomeEmpresa(c?.nome))
                .filter(n => n.length >= 4)
            );
            const ehConcorrenteMapeado = (fornecedor: unknown): boolean => {
              const f = normalizarNomeEmpresa(fornecedor);
              if (f.length < 4) return false;
              if (nomesConcorrentes.has(f)) return true;
              for (const n of nomesConcorrentes) {
                if (n.length >= 8 && f.length >= 8 && (f.includes(n) || n.includes(f))) return true;
              }
              return false;
            };
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm flex flex-col gap-6">
            ...
            {/* ⚠️ O erro fica ACIMA das abas, no nível do componente.
                A reapuração é UMA requisição que traz nacionais e regionais
                juntos. Antes esta mensagem morava dentro do estado vazio, que só
                renderiza quando a aba ativa está sem dados — com nacionais
                preenchidos e regionais vazios, o mesmo erro ficava invisível numa
                aba e visível na outra, dando a impressão de que a falha era "só
                dos regionais". A tela é que mentia. */}
            {erroReapuracao && (
              <div className="flex items-start gap-3 px-4 py-3 rounded-2xl bg-red-50 border border-red-200">
                <AlertTriangle size={16} className="text-red-500 shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="text-xs font-bold text-red-800">Não foi possível atualizar o radar</p>
                  <p className="text-xs text-red-700 mt-0.5 leading-relaxed">{erroReapuracao}</p>
                  <p className="text-[10px] text-red-500 mt-1">
                    Vale para as duas listas — nacionais e regionais vêm da mesma consulta.
                  </p>
                </div>
                <button
                  onClick={() => setErroReapuracao(null)}
