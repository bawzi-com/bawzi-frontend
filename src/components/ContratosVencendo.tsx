...
// Explicação de cada janela do seletor de período.
// Fica junto de urgencia() de propósito: as duas usam as mesmas faixas de dias
// (15/30/60/90) e quem mexer numa precisa enxergar a outra. Atenção à distinção
// que confunde: o BOTÃO escolhe quanto à frente olhar; a COR do cartão classifica
// a urgência dentro da janela escolhida. Números iguais, significados diferentes.
const DIAS_INFO: Record<number, string> = {
  30: 'Vence em até 30 dias — faixas CRÍTICO (vermelho) e URGENTE (laranja). Se o órgão ainda não publicou a nova licitação, ela sai nas próximas semanas. Janela de reação, não de preparação.',
  60: 'Até 60 dias — acrescenta a faixa ATENÇÃO (âmbar). É quando o órgão costuma montar o termo de referência: ainda dá tempo de influenciar especificação e reunir documentação.',
  90: 'Até 90 dias — acrescenta a faixa RADAR (verde). Prazo confortável para cotar com fornecedor, revisar preço e chegar na disputa sem correria.',
  180: 'Até 6 meses — entra a faixa PIPELINE (azul). Serve para mapear quem é o fornecedor atual e a que preço ele venceu, antes de o mercado olhar.',
  365: 'Até 1 ano — visão de carteira: quanto do seu mercado renova no período e em que meses a renovação se concentra.',
  730: 'Até 2 anos — horizonte máximo. Pega os contratos longos, que costumam ser os de maior valor e os que exigem preparação mais cedo.',
};

// O que cada cor do prazo restante significa.
// Chaveado pelo `label` que urgencia() já devolve de propósito: assim as faixas
// de dias continuam definidas num lugar só (lá em cima) e aqui fica só o texto.
// Mexeu no label de lá? Mexa na chave daqui.
const EXPLICACAO_URGENCIA: Record<string, string> = {
  Indefinido: 'Sem data de fim de vigência publicada no PNCP. Não dá para calcular o prazo — vale abrir o contrato e conferir na fonte.',
  VENCIDO:    'VENCIDO (cinza) — a vigência já terminou. Ou o órgão renovou sem publicar, ou o contrato caducou e a nova licitação deve estar próxima.',
  HOJE:       'HOJE (vermelho) — vence hoje. Sem licitação publicada até agora, a contratação seguinte tende a sair como emergencial ou dispensa.',
  'CRÍTICO':  'CRÍTICO (vermelho) — vence em até 15 dias. Curto demais para montar proposta do zero; serve para vigiar se sai dispensa ou emergencial.',
  URGENTE:    'URGENTE (laranja) — vence em até 30 dias. Se a nova licitação vier, sai nas próximas semanas. Documentação e preço já precisam estar prontos.',
  'ATENÇÃO':  'ATENÇÃO (âmbar) — vence em até 60 dias. Janela típica de montagem do termo de referência: ainda dá tempo de influenciar a especificação.',
  RADAR:      'RADAR (verde) — vence em até 90 dias. Prazo confortável para cotar com fornecedor e revisar preço antes da disputa.',
  PIPELINE:   'PIPELINE (azul) — vence em mais de 90 dias. Fase de planejamento: mapeie o fornecedor atual e a que preço ele venceu, antes de o mercado olhar.',
};
          ...
          if (listaOrdenada.length === 0) {
            return (
              <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
                <span className="text-4xl">⏳</span>
                <p className="text-sm font-bold text-slate-600">
                  Nenhum contrato recorrente nesta janela
                </p>
                <p className="text-[11px] text-slate-400 font-medium max-w-sm">
                ...
                >
                  Mostrar também os emergenciais
                </button>
              </div>
            );
          }
          ...
          <>
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400">
                {/* ⚠️ UM BOTÃO, E NÃO TRÊS CHIPS DE TEMPO.
                    Isto era "TODAS · ≥6M · ≥12M" e colidia com o seletor de
                    janela do topo (30D 60D 90D...): dois controles de período
                    com a mesma cara, medindo eixos diferentes — um é QUANDO
                    vence, o outro é QUANTO durou — e nada dizia qual era qual.

                    Ninguém olha esta tela querendo "contratos de 6 meses ou
                    mais". Quer saber se aquilo volta a ser disputado ou se foi
                    compra de emergência. Seis meses é a implementação; estava
                    no rótulo ocupando o lugar do conceito. */}
                <button
                  type="button"
                  title="Esconde contratos com menos de 6 meses de vigência — normalmente compra emergencial ou dispensa, que não volta como licitação. Contratos sem duração informada continuam na lista."
                  onClick={() => { setDuracaoMin(duracaoMin ? 0 : 6); setPagina(1); }}
                    ...
                    <div className={`group relative rounded-2xl p-5 hover:shadow-md transition-all cursor-default border-l-4 ${u.border} ${
                      ...
                      <div className={`flex items-start justify-between gap-3 mb-2 ${isOp ? 'mt-1' : ''}`}>
                        <div className="flex items-center gap-2 min-w-0">
                          <span
                            title={EXPLICACAO_URGENCIA[u.label] ?? u.label}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span
                            title={EXPLICACAO_URGENCIA[u.label] ?? u.label}
                            className={`text-[10px] font-black px-2.5 py-1 rounded-lg whitespace-nowrap cursor-help ${u.badge}`}
                          >
                            {c.dias_restantes == null
                              ...
                              : c.dias_restantes < 0
                                ...
                                : c.dias_restantes === 0
                                  ? 'HOJE'
                                  /* mostra o rótulo da faixa junto do prazo: sem isto
                                     o cartão exibe só "45d" em âmbar e a cor não se
                                     explica. O rótulo já era calculado e descartado. */
                                  : `${c.dias_restantes}d · ${u.label}`}
                          </span>
                        </div>
                      </div>
