...
import ComprarCreditosModal, { type PacoteInfo } from './ComprarCreditosModal';
...
export default function AnalysisApp() {
  ...
  // ─── useEffect: volta do checkout de créditos ───────────────────────────────
  // O webhook do Stripe é o caminho oficial de creditar, mas ele só funciona
  // se o Stripe CONSEGUIR alcançar este servidor. Em desenvolvimento (localhost
  // sem `stripe listen`), atrás de firewall, ou com o endpoint fora do ar por
  // um minuto, a entrega falha calada e quem PAGOU volta para uma tela com o
  // saldo velho — foi exatamente isso que aconteceu.
  //
  // Aqui a pergunta é feita na direção oposta: o redirect traz o `session_id`,
  // nós perguntamos ao Stripe se foi pago e creditamos. Os dois caminhos usam
  // a MESMA função no backend, com a trava dentro do filtro do update, então
  // podem chegar juntos sem creditar em dobro.
  ...
  useEffect(() => {
    ...
    // Lê ANTES de limpar. (Na primeira versão deste bloco eu apaguei os
    // parâmetros e só então tentei ler o session_id — que já não existia.)
    const sessionId = url.searchParams.get('session_id') || '';

    // Limpa a query string logo em seguida, antes de qualquer await: sem isso
    // um F5 na tela de retorno re-dispara a confirmação, e o histórico do
    // navegador fica guardando um id de sessão de pagamento na URL.
    url.searchParams.delete('pacote');
    url.searchParams.delete('session_id');
    window.history.replaceState({}, '', url.pathname + url.search + url.hash);

    if (status === 'cancelado') {
      setAvisoPacote('Compra cancelada — nada foi cobrado.');
      return;
    }
    ...
    (async () => {
      try {
        ...
        // Recarrega o saldo nos DOIS casos: se o webhook já tinha creditado, o
        // número na tela continua o antigo até esta chamada.
        apiFetch(`${API_URL}/api/analyses/quota`)
          .then(r => r.ok ? r.json() : null)
  ...
  const confirmShare = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(mensagemDeErro(errorData?.detail, 'Falha ao partilhar o relatório.'));
      }
   ...
   *
   *  Existe porque "Fazer upgrade" não é resposta para quem já está no plano
   *  de topo, nem para quem só quer resolver o mês corrente. O crédito é dado
   *  pelo webhook do Stripe ao confirmar o pagamento, nunca aqui. */
  // ── Compra de créditos avulsos ────────────────────────────────────────
  // O `window.prompt` que existia aqui não mostrava quantos créditos o valor
  // virava enquanto a pessoa digitava, não traduzia crédito em nada
  // reconhecível e só validava a faixa no servidor. Virou modal.
  ...
  const handleComprarPacote = async () => {
    ...
    try {
      ...
      window.alert('Não foi possível consultar o preço do crédito agora.');
    }
  };

  const confirmarCompraPacote = async (valorBRL: number) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        window.alert(mensagemDeErro(data?.detail, 'Não foi possível abrir o checkout agora.'));
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans overflow-x-hidden relative">
      ...
      {/* Resultado da volta do checkout. Sem isto, a confirmação acontece em
          silêncio e a pessoa não sabe se o crédito entrou por esta via ou se
      ...
      <ComprarCreditosModal
        ...
        onConfirmar={confirmarCompraPacote}
      />
              ...
              {activeTab === 'cnae' && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {/* Aberta a qualquer conta. `/feed-cnae` vive no router_pncp,
                      que não tem uma única chamada de modelo e ainda serve de
                      cache — travar isto no Nível 3 não economizava nada e
                      escondia justamente a tela que cria hábito diário. */}
                  {(
                    <CnaeOportunidades
                      token={token}
                      userData={userData}
