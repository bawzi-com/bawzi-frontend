...
export default function CompanyProfileForm({ companyData, userTier, token, onUpdate, onCnpjDetected }: any) {
  ...
  // ⚠️ O Nível 1 tinha ZERO vagas, e isso travava por tabela duas
  // funcionalidades que acabaram de ser abertas: Oportunidades casa o edital
  // pelo CNAE da empresa e Renovações lista os contratos a vencer DELA. Sem
  // vaga, as duas apareciam e não tinham o que mostrar — pior que continuarem
  // fechadas, porque a promessa fica na tela e o resultado não vem.
  //
  // Cadastrar empresa não chama modelo nenhum (`router_company.py` não tem uma
  // linha de IA; o CNPJ vem da BrasilAPI, pública), então a vaga cabe no mesmo
  // critério de custo zero que decidiu as aberturas.
  //
  // A régua passa a ser QUANTAS, não SE: 1 / 1 / 2 / 3. Quem toca uma empresa
  // não precisa de plano por causa disto; quem toca três, precisa.
  const vagasTotais = userTier === 4 ? 3 : userTier === 3 ? 2 : userTier >= 1 ? 1 : 0;
  ...
  const handleFetchCnpj = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/api/company/search/${cnpjLimpo}`);
      const data = await res.json();
      if (!res.ok) throw new Error(mensagemDeErro(data.detail, 'Erro ao buscar CNPJ'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(mensagemDeErro(resData.detail, 'Erro ao processar dados'));
