...
export default function ImpugnacaoModal({
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-4xl max-h-[90dvh] rounded-[2rem] flex flex-col shadow-2xl relative overflow-hidden">

        {/* Cabeçalho */}
        <div className="p-6 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 bg-slate-50">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={onCopy}
              className="px-4 py-2.5 min-h-10 bg-white hover:bg-slate-100 text-slate-700 font-bold rounded-lg text-xs transition-colors border border-slate-200 shadow-sm"
            >
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 shrink-0 flex items-center justify-center rounded-full bg-slate-200 text-slate-600 hover:bg-red-100 hover:text-red-600 transition-colors"
            >
              ✖
            </button>
          </div>
        ...
        <div className="p-4 sm:p-8 overflow-y-auto flex-1 bg-slate-100/50">
          <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 sm:p-8 max-w-3xl mx-auto font-serif text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">
            {impugnacaoText}
          </div>
        </div>
