...
export interface AnalysisResult {
   ...
   *  cada campo é opcional e a seção some inteira quando não há dado. */
  auditoria?: {
    /** Divergências entre trechos que falam do mesmo assunto, ou entre o
     *  edital e o cadastro do PNCP. É o achado que a rápida não produz. */
    contradicoes?: {
      tipo?: string;
      assunto?: string;
      gravidade?: string;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    cobertura?: Record<string, any>;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    qualidade_extracao?: Record<string, any>;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    fatos?: any[];
  };
  prazos?: string[];
