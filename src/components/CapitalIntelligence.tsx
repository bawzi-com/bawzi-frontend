        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-0.5">
              ...
              {isRealApi ? (
                <span
                  title="Resposta real da API da instituição para este CNPJ — não é simulação"
                  className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-violet-50 border border-violet-200 text-violet-700"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse inline-block" />
                  API AO VIVO
                </span>
              ) : (
                /* Sem este rótulo, o cartão simulado era visualmente idêntico
                   ao da consulta real — e "elegível" simulado lido como
                   aprovação real queima a confiança quando o banco nega. */
                <span
                  title="Compatibilidade estimada pela Bawzi com dados públicos do CNPJ — esta instituição NÃO foi consultada"
                  className="inline-flex items-center text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-500"
                >
                  Estimativa
                </span>
              )}
...
export default function CapitalIntelligence({ token, defaultCnpj = '', defaultUf = '', companies = [], tier = 3, defaultValorEdital, defaultObjeto }: Props) {
  ...
  // Erro por campo, separado do `erro` geral (que é para falha de rede/API).
  // Antes, "Informe o valor estimado" aparecia numa caixa acima do botão,
  // longe do campo que o causou — o utilizador lia a queixa sem ver o culpado.
  const [erroCampo, setErroCampo]         = useState<{ cnpj?: string; valor?: string }>({});
  ...
  async function buscar() {
    ...
    const problemas: { cnpj?: string; valor?: string } = {};
    if (cnpjLimpo.length !== 14) problemas.cnpj = 'Informe um CNPJ válido com 14 dígitos.';
    if (valor <= 0) problemas.valor = 'Informe o valor estimado do edital.';
    setErroCampo(problemas);
    if (problemas.cnpj || problemas.valor) return;
    ...
    try {
      ...
      if (!resRec.ok) {
        ...
        throw new Error(mensagemDeErro(err.detail, `Erro ${resRec.status}`));
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 space-y-4">
        <div>
          ...
          {/* Sem isto o utilizador entrega um valor e espera por um botão que
              promete crédito, sem saber se recebe um número, um parecer ou uma
              lista de bancos. Os quatro itens são exatamente os blocos que a
              secção de resultados desenha abaixo. */}
          <p className="mt-1 text-[11px] font-medium text-slate-400">
            Devolve capital de giro necessário, score de crédito, estratégia recomendada
            e simulação de elegibilidade em instituições.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            {hasCompanies && !useCustomCnpj ? (
              ...
              <div className="relative">
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={cnpj}
                  onChange={e => { setCnpj(formatCnpj(e.target.value)); if (erroCampo.cnpj) setErroCampo(p => ({ ...p, cnpj: undefined })); }}
                  placeholder="00.000.000/0000-00"
                  aria-invalid={!!erroCampo.cnpj}
                  className={`w-full pl-9 pr-4 py-3 rounded-xl border text-sm font-mono focus:outline-none focus:ring-2 focus:border-transparent ${
                    erroCampo.cnpj
            ...
            {erroCampo.cnpj && (
              <p className="mt-1.5 text-[11px] font-bold text-red-600">{erroCampo.cnpj}</p>
            )}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                inputMode="decimal"
                value={valorEdital}
                onChange={e => { setValor(formatValor(e.target.value)); if (erroCampo.valor) setErroCampo(p => ({ ...p, valor: undefined })); }}
                placeholder="0,00"
                aria-invalid={!!erroCampo.valor}
                className={`w-full pl-9 pr-4 py-3 rounded-xl border text-sm font-mono focus:outline-none focus:ring-2 focus:border-transparent ${
                  erroCampo.valor
                    ? 'border-red-300 bg-red-50/40 focus:ring-red-300'
                    : 'border-slate-200 focus:ring-emerald-400'
            ...
            {erroCampo.valor && (
              <p className="mt-1.5 text-[11px] font-bold text-red-600">{erroCampo.valor}</p>
            )}
          </div>
        </div>
