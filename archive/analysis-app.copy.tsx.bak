              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                ...
                {loadingHistory ? (
                  ...
                  <div className="relative">
                    {/* 🟢 Variáveis de Segurança (Evita que o React crashe a página inteira) */}
                    {(() => {
                      // Mude 'selectedTier' para o nome da variável que guarda o nível do utilizador no seu ficheiro
                      const nivelAtual = typeof selectedTier !== 'undefined' ? selectedTier : 0;
                      ...
                      return (
                        <>
                          {/* CONTAINER COM BLUR */}
                          <div className={isLocked ? 'blur-[5px] opacity-40 select-none pointer-events-none transition-all duration-300' : ''}>
                            
                            {(isLocked || hasHistory) ? (
                              <div className="space-y-3">
                                {(isLocked ? [1, 2, 3] : (history || [])).map((h: any, i: number) => (
                                  <a
                                    key={i}
                                    href={!isLocked ? h.link : '#'}
                                  ...
                                  >
                                    ...
                                    <div className="pr-6">
                                      <span className="block text-[10px] font-black text-slate-500 uppercase truncate mb-1.5">
                                        {isLocked ? 'Prefeitura Municipal / Governo do Estado' : h.orgao}
                                      </span>
                                      <p className="text-xs text-slate-700 font-medium line-clamp-2 leading-relaxed mb-3">
                                        {isLocked ? 'Contrato de fornecimento de materiais e prestação de serviços continuados sob o número...' : h.objeto}
                                      </p>
                                      <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
                                        <span className="text-[9px] font-bold text-slate-400 flex items-center gap-1.5">
                                          📅 {isLocked ? 'Vence nos próximos dias' : (h.data ? new Date(h.data).toLocaleDateString('pt-BR') : 'Sem data')}
                                        </span>
                                        {isLocked || h.valor > 0 ? (
                                          <span className="text-[10px] font-black text-emerald-700 bg-emerald-50 px-2 py-1 rounded border border-emerald-100 flex items-center gap-1">
                                            💰 {isLocked ? 'R$ ***.***,**' : new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(h.valor)}
                                          </span>
                                        ) : (
                          ...
                          {isLocked && (
                            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-4">
                              <div className="bg-white/95 p-6 rounded-2xl shadow-2xl border border-indigo-100/50 flex flex-col items-center max-w-sm text-center backdrop-blur-sm">
                                <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mb-4 ring-4 ring-white shadow-sm">
                                  <Lock size={20} />
                                </div>
                                <h4 className="text-sm font-black text-slate-800 mb-2 flex items-center gap-1.5 justify-center w-full">
                                  <Crown size={16} className="text-amber-500" />
                                  Inteligência Corporativa
                                </h4>
                                <p className="text-xs text-slate-500 mb-5 font-medium leading-relaxed px-2">
                                  Descubra os contratos milionários que os seus concorrentes estão prestes a perder. Acesso exclusivo para o <strong>Nível 4 (Dominador)</strong>.
                                </p>
                                
                                <button 
                                  onClick={() => setShowUpsell(true)}
                                  className="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white text-[11px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center justify-center gap-2"
                                >
                                  Desbloquear Acesso
                                </button>
                              </div>
